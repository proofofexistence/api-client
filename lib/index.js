...
const request = function(options) {
  ...
  // set default baseURL to main site
  if (Object.keys(options).indexOf("baseURL") === -1)
    options.baseURL = 'https://proofofexistence.com'
 ...
 */
const getLatestConfirmed = (config={}) => {
  return request( Object.assign({}, config,
    {
      method: 'get',
 ...
 * @returns {Promise.Array.<Registration>} an array of confirmed
 */
const getLatestUnconfirmed = (config={}) => {
  return request( Object.assign({}, config,
    {
      method: 'get',
      url: getURL('latest/unconfirmed', true)
    }
  ))
}
 ...
 * @returns {Promise.<Registration>}
 */
const updateStatus = (hash, config={}) => {
  return request( Object.assign({}, config,
    {
      method: 'post',
      url: getURL('status'),
